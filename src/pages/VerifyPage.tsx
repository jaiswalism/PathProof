...
import { Shield, ChevronRight, Loader2, Package, Clock, MapPin, BadgeCheck, Image as ImageIcon } from 'lucide-react';
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      const verificationPromises = metadata.proofs.map(proof =>
        verifyProof(proof.proofHash, productId, proof.checkpoint, proof.location)
      );
          ...
          <div className="card mt-8">
              ...
              {verified ? (
                ...
                >
                  {verifying ? (
                    ...
                    <>
                      <Shield size={18} className="mr-2" /> Verify
                    </>
                  )}
                </button>
              )}
            ...
            <div className="mb-8 grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div>
                ...
                <p className="font-medium">{new Date(metadata.manufactureDate).toLocaleDateString()}</p>
              </div>
            </div>
                      ...
                      <div key={index} className="relative">
                        <div className="flex">
                          <div className="mr-4 flex flex-col items-center">
                            <div className={`flex h-10 w-10 items-center justify-center rounded-full ${verified ? 'bg-green-100 text-green-600' : 'bg-teal-100 text-teal-600'}`}>
                              {index === 0 ? <Package size={20} /> : <BadgeCheck size={20} />}
                            </div>
                            {!isLast && <div className="my-1 h-full w-0.5 bg-slate-300"></div>}
                          </div>
